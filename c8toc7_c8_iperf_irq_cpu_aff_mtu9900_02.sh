...
SLEEP="30"
HOSTNAME="c8"
TESTFILENAME="$(date +%F_%H-%M-%S)_c8toc7_${HOSTNAME}_iperf_irq_cpu_aff_mtu${MTUSIZE}_02.txt"
...
BW=$(echo -n $(taskset -c 0 iperf -c $IP_SRV -t 30  -y C | awk -F',' '{print $9}'))
...
IRQ4=$(echo  -n $(awk '/.*ens2-4$/ {print $2}  ' /proc/interrupts))     
IRQ5=$(echo  -n $(awk '/.*ens2-5$/ {print $3}  ' /proc/interrupts))     
IRQ6=$(echo  -n $(awk '/.*ens2-6$/ {print $4}  ' /proc/interrupts))     
IRQ7=$(echo  -n $(awk '/.*ens2-7$/ {print $5}  ' /proc/interrupts))     
